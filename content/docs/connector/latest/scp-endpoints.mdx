...
Connecting to Klave is done through the `SCP` class of the connector.
The `SCP` class is the main entry point to connect to Klave, provides a set of methods to connect to Klave,
manage the connection and interact with your Klave applications.


The connection api of the `SCP` is the `connect` method, which needs a Klave endpoint and
a cryptographic key. You can either import an existing key or create a new one through our `Key` class.

## Connecting to Klave with an ephemeral key
