...
**Attestation Process**: Before processing begins, clients can verify that:
...
**Bring Your Own Model (BYOM)**: Model owners can deploy proprietary models by:
...
<Image className="block dark:hidden" src="/resources/klave-ai-private-rag.svg" alt="Klave AI Overview" style={{margin: "auto", padding: "4rem 0 4rem 0"}} height={700} width={700} />
...
Klave AI Agents communicate leveraging an Attested version of the Agent-to-Agent (A2A) protocol called Secure A2A (SA2A).

#### SA2A (Secure Agent-to-Agent) Protocol
...
In addition of running within secure enclaves, they also enhance the MCP by communication with AI Agents trough an Attested MCP names SMCP.

#### Enhanced MCP Protocol (SMCP - Secure MCP):
