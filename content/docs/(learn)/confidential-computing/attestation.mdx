...
The remote attestation process for hardware-based [TEEs](./tee) provides evidence of the loaded logic and the state of the platform.
...
The enclave will be using MRENCLAVE [Sealing](/confidential-computing/sealing) to encrypt the data before requesting the host to persist it and unsealing it when requested to perform the computation.
