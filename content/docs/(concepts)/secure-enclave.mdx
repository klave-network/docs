...
The cornerstorne of [Confidential Computing](/confidential-computing/overview) and a key concept behind Klave are the secure hardware enclaves. Actually the name Klave comes from the word enclave (on-klave, enclave, get it?).
Klave leverage [Trusted Execution Environments (TEEs)](/confidential-computing/tee) to run your workloads, ensuring that your business logic and data stay confidential and tamper-proof at all times.
...
TEEs are hardware based and platform related. Klave leverage on [Intel SGX](/confidential-computing/tee#intel-sgx) for the TEEs.
...
The Klave platform, through the [Klave SDK](/sdk/latest/create-query-transaction), addresses these limitations by providing essential capabilities to enclaves, including network calls, ledger access, cryptographic primitives, etc.
