...
<Tabs groupId="language" items={['AssemblyScript', 'Rust']}>
    ...
    <Tab value="Rust">
        ...
        |`ledger`| `get` | key | value | Get the string value associated with the specified `key` from the specified `Table` as string. |
        |`ledger`| `set` | key, value | - | Set the `value` as string associated with the specified `key` in the specified `Table`, overwriting any existing value. |
        |`ledger`| `unset` | key | - | Delete the tuple with the specified `key` from the specified `Table`. |
