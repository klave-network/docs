...
<Tabs groupId="language" items={['AssemblyScript', 'Rust']}>
    <Tab value="AssemblyScript">
        ...
        export class HmacKeyGenParams {
            ...
            length: u32 = 0; // Size in bits. Optional - if not set, keys are automatically sized to the block size of the sha algorithm.
        }
        ```
        ```js copy
        import { Crypto } from '@klave/sdk';
    ...
    <Tab value="Rust">
        ...
        pub struct HmacKeyGenParams {
            pub hash: String, // "SHA2-256", "SHA2-384", "SHA2-512"
            pub length: u32, // Size in bits. Optional - if not set, keys are automatically sized to the block size of the sha algorithm.
        }
        ```
        ```rust
        use klave::{self, crypto::subtle, crypto::subtle::CryptoKey};
...
<Tabs groupId="language" items={['AssemblyScript', 'Rust']}>
    <Tab value="AssemblyScript">
        | Class  | Operation  | Parameters: type | Returns | Behavior |
        |--------|------------|------------------|---------|----------|
        |`Crypto.Subtle`| `deriveKey` | `algorithm: T, baseKey: CryptoKey, derivedKeyAlgorithm: E, extractable: boolean, usages: string[]`| `Result<CryptoKey, Error>` | Derive a `CryptoKey` from the `baseKeyName` using the specified `algorithm` and `derivedKeyAlgorithm`. Return a `CryptoKey` or an `Error`.|
        |`Crypto.Subtle`| `deriveBits` | `algorithm: T, baseKey: CryptoKey, length: u32`| `Result<ArrayBuffer, Error>` | Derive raw bits from the `baseKey` using the specified `algorithm` and desired bit `length`. Return an `ArrayBuffer` containing the derived bits or an `Error`.|
    ...
    <Tab value="Rust">
        ...
        |`crypto::subtle`| `derive_bits` | `derivation_algorithm: KeyDerivationAlgorithm, base_key: CryptoKey, length: u32` | `Result<Vec<u8>, Error>`  | Use the `derivation_algorithm` to derive raw bits from the `base_key` with the specified bit `length`. Return the derived bits as a `Vec<u8>` or an `Error`.|
...
<Tabs groupId="language" items={['AssemblyScript', 'Rust']}>
    <Tab value="AssemblyScript">
        ...
        */
        ...
        You can also derive raw bits directly using `deriveBits`:
        ...
        * @query
        */
        export function subtleCryptoECDH_Bits(): void
        {
            // Generate EC key pair
            let eccPrivateKeyResult = Crypto.Subtle.generateKey({namedCurve: "P-256"} as Crypto.EcKeyGenParams, true, ["deriveBits"]);
            let eccPrivateKey = eccPrivateKeyResult.data as Crypto.CryptoKey;
            ...
            let ecdhParams = {publicKey: eccPublicKey} as Crypto.EcdhKeyDeriveParams;
            let derivedBitsResult = Crypto.Subtle.deriveBits(ecdhParams, eccPrivateKey, 256); // 256 bits = 32 bytes
            let derivedBits = derivedBitsResult.data as ArrayBuffer;
        }
        ```
    </Tab>
    <Tab value="Rust">
        ...
        You can also derive raw bits directly using `derive_bits`:

        ```rust
        use klave::{self, crypto::subtle, crypto::subtle::CryptoKey};

        fn subtle_crypto_ecdh_bits(input: String) {
            // Generate EC private key
            let ec_params = subtle::EcKeyGenParams { named_curve: "P-256".to_string() };
            let key_usages_slice: Vec<&str> = vec!["deriveBits"];
            let gen_algorithm = subtle::KeyGenAlgorithm::Ecc(ec_params);
            let private_key: CryptoKey = match subtle::generate_key(&gen_algorithm, true, &key_usages_slice) {
                Ok(result) => result,
                Err(err) => {
            ...
            let derived_bits: Vec<u8> = match subtle::derive_bits(&derivation_algorithm, &private_key, 256) { // 256 bits = 32 bytes
                Ok(result) => result,
                Err(err) => {
                    klave::notifier::send_string(&err.to_string());
                    return;
...
<Tabs groupId="language" items={['AssemblyScript', 'Rust']}>
    <Tab value="AssemblyScript">
        ...
        * @query
        */
        export function subtleCryptoHKDF_Bits(): void
        {
            // Generate base key material
            let baseKeyResult = Crypto.Subtle.generateKey({length: 256} as Crypto.AesKeyGenParams, true, ["deriveBits"]);
            let baseKey = baseKeyResult.data as Crypto.CryptoKey;

            // Prepare HKDF parameters
            let salt = Crypto.getRandomValues(32); // 32 bytes salt
            let info = String.UTF8.encode("raw-bits-derivation"); // Context info
            let hkdfParams = {
                hash: "SHA2-256",
                salt: salt.buffer,
                info: info
            ...
            let derivedBitsResult = Crypto.Subtle.deriveBits(hkdfParams, baseKey, 512); // 512 bits = 64 bytes
            let derivedBits = derivedBitsResult.data as ArrayBuffer;
        }
        ```
    </Tab>
    <Tab value="Rust">
        ...
        fn subtle_crypto_hkdf_bits(input: String) {
            // Generate base key material
            let aes_params_base = subtle::AesKeyGenParams { length: 256 };
            let key_usages_base: Vec<&str> = vec!["deriveBits"];
            let gen_algorithm_base = subtle::KeyGenAlgorithm::Aes(aes_params_base);
            let base_key: CryptoKey = match subtle::generate_key(&gen_algorithm_base, true, &key_usages_base) {
                Ok(result) => result,
                Err(err) => {
            ...
            let info = "raw-bits-derivation".as_bytes().to_vec();
            ...
            // Derive raw bits using HKDF
            let derived_bits: Vec<u8> = match subtle::derive_bits(&derivation_algorithm, &base_key, 512) { // 512 bits = 64 bytes
                Ok(result) => result,
                Err(err) => {
                    klave::notifier::send_string(&err.to_string());
                    return;
